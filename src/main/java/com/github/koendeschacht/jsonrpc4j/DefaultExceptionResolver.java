...
public class DefaultExceptionResolver implements ExceptionResolver {
	...
	public Throwable resolveException(ObjectNode response) {
		...
		if (!Util.hasNonNullTextualData(dataObject, EXCEPTION_TYPE_NAME))
			return createJsonRpcClientException(errorObject);
		
		try {
			String exceptionTypeName = dataObject.get(EXCEPTION_TYPE_NAME).asText();
			String message = Util.hasNonNullTextualData(dataObject, ERROR_MESSAGE) ? dataObject.get(ERROR_MESSAGE).asText() : null;
